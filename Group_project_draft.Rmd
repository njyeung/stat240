...
Question
```{r}
crime_data<-crime_data%>%mutate(date_crime=substring(Date.Rptd,1,10))
crime_data
...
```
```{r}
crime_data%>%group_by(date)%>%summarise(n=n())%>%ggplot(aes(x=date,y=n),color="blue")+geom_line()+geom_smooth()+geom_smooth(method = "lm",se=FALSE,color="red")+labs(x="date",y="how many crimes happened this day",title = "crime each day change over time")
```
...
Methods
Compute counts by using group_by() and count(),also use arrange()to make the data into sequence
Confidence intervals for differences in means between years.
Potential thesis
Crime incidents have increased form 2000 tio 2022,however, there is a steadily decrease of crime incidence starting from 2024.and the overall change of crime from 2020 to 2024 is decreasing.
...
```{r}
crime_data%>%group_by(AREA.NAME)%>%count()%>%arrange(desc(n))%>%ggplot(aes(x=AREA.NAME,y=n))+geom_bar(stat = "identity")
```
...
```{r}
crime_filtered<-crime_data%>%count(Vict.Sex)%>%filter(Vict.Sex=="M"|Vict.Sex=="F")
crime_filtered
crime_filtered%>%ggplot(aes(x=Vict.Sex,y=n,fill = Vict.Sex),)+geom_bar(stat = "identity")+labs(x="sex",y="the number of crimes by each gender",title = "number of crimes convicted by male and femals")
```
Methods
